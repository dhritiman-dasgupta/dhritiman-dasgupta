...
| **[jetson-avatar-studio](https://github.com/dhritiman-dasgupta/jetson-avatar-studio)** | A talking avatar running entirely on an 8 GB Jetson Orin — Whisper STT, a llama.cpp LLM and cloned-voice TTS behind a C++ orchestrator. The q8 TTS plus a GPU codec does not fit in 8 GB, so the codec is pinned to CPU; ~8 s is the Orin's own inference floor. |
